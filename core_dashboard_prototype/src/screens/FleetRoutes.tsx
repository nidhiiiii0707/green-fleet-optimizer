...
const STATUS_STYLE: Record<string, { bg: string; color: string }> = {
  "on-schedule": { bg: "var(--gf-green-soft)", color: "#39B875" },
  warning:       { bg: "var(--gf-amber-soft)", color: "#D99A31" },
  critical:      { bg: "var(--gf-red-soft)", color: "#E45B61" },
};

export default function FleetRoutes({ solutionId, result: liveResult, loading: optimizationLoading, error: optimizationError }: { solutionId: string; result: OptimizationResult | null; loading: boolean; error: string | null }) {
  ...
  return (
    <div style={{ display: "flex", height: "100%", overflow: "hidden" }}>
      {/* Filter sidebar */}
      <div style={{ width: 200, background: "var(--gf-card)", borderRight: "1px solid var(--gf-line)", padding: "16px", overflowY: "auto", flexShrink: 0 }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: "var(--gf-ink)", marginBottom: 12 }}>Filters</div>
        ...
        {[
          ...
          <div key={f.label} style={{ marginBottom: 16, opacity: f.disabled ? 0.45 : 1 }}>
            <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.07em", fontWeight: 700, color: "var(--gf-faint)", marginBottom: 6 }}>{f.label}</div>
            {f.disabled && (
              <div style={{ fontSize: 10, color: "var(--gf-faint)", marginBottom: 6 }}>Not tracked by the current fleet data.</div>
            )}
            {f.options.map(opt => (
              <label key={opt} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4, cursor: f.disabled ? "not-allowed" : "pointer" }}>
                <input
                  ...
                  disabled={f.disabled}
                  onChange={() => f.set(opt)}
                  style={{ accentColor: "#16A3A8" }}
                />
                <span style={{ fontSize: 12, color: f.value === opt ? "#26B7BA" : "var(--gf-muted)", fontWeight: f.value === opt ? 600 : 400 }}>
                  {({ "on-schedule": "On Schedule", "in-transit": "In Transit", available: "Available", maintenance: "Maintenance" } as Record<string, string>)[opt] ?? opt}
                </span>
              </label>
            ))}
          </div>
        ...
        <div style={{ borderTop: "1px solid var(--gf-line)", paddingTop: 12 }}>
          <div style={{ fontSize: 11, color: "var(--gf-faint)" }}>{displayRows.length} of {assignments.length} assignments shown</div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        {/* Map */}
        <div style={{ background: "var(--gf-card)", borderBottom: "1px solid var(--gf-line)", height: 260, flexShrink: 0 }}>
          <GoogleFleetMap
            ports={PORTS}
            routes={ROUTES}
        ...
        {/* Table */}
        <div style={{ flex: 1, overflowY: "auto", background: "var(--gf-card)" }}>
          <div style={{ padding: "12px 16px", borderBottom: "1px solid var(--gf-line)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: "var(--gf-ink)" }}>Fleet Assignment Table</div>
            <span style={{ fontSize: 12, color: "var(--gf-faint)" }}>Click a row to view vessel details</span>
          </div>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <thead>
              <tr style={{ background: "var(--gf-row-alt)", position: "sticky", top: 0, zIndex: 1 }}>
                {["Vessel", "Cargo", "Origin", "Destination", "Route", "Speed", "Fuel", "Shore Power", "ETA", "Fuel (t)", "Cost", "GHG", "Status"].map(c => (
                  <th key={c} style={{ padding: "9px 10px", textAlign: "left", fontSize: 10, fontWeight: 700, color: "var(--gf-muted)", textTransform: "uppercase", letterSpacing: "0.05em", borderBottom: "1px solid var(--gf-line)", whiteSpace: "nowrap" }}>{c}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {displayRows.map((row, i) => {
                ...
                const ss = STATUS_STYLE[row.status] ?? { bg: "var(--gf-card)", color: "var(--gf-muted)" };
                const route = ROUTES.find(r => r.id === row.routeId);
                return (
                  <tr
                    key={row.id}
                    onClick={() => { setSelectedVesselId(row.vesselId); setDrawerOpen(true); }}
                    style={{ background: isSelected ? "var(--gf-selected)" : i % 2 === 0 ? "var(--gf-card)" : "var(--gf-row-alt)", cursor: "pointer", borderBottom: "1px solid var(--gf-line)" }}
                  ...
                  >
                    <td style={{ padding: "8px 10px", fontWeight: 600, color: isSelected ? "#2DC4C7" : "var(--gf-ink)", whiteSpace: "nowrap" }}>{row.vesselName.replace("MV ", "")}</td>
                    <td style={{ padding: "8px 10px", color: "var(--gf-muted)" }}>{row.cargo}</td>
                    <td style={{ padding: "8px 10px", color: "var(--gf-muted)" }}>{row.originId}</td>
                    <td style={{ padding: "8px 10px", color: "var(--gf-muted)" }}>{row.destinationId}</td>
                    <td style={{ padding: "8px 10px", color: "var(--gf-muted)", fontSize: 11, maxWidth: 120, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{route?.name ?? row.routeId}</td>
                    <td style={{ padding: "8px 10px", fontFamily: "'JetBrains Mono', monospace", color: "var(--gf-muted)" }}>{row.speed} kn</td>
                    <td style={{ padding: "8px 10px" }}>
                      <span style={{ background: "var(--gf-info-soft)", color: FUEL_COLORS[row.fuelType] ?? "#48A6FF", borderRadius: 4, padding: "1px 6px", fontSize: 10, fontWeight: 700 }}>{row.fuelType}</span>
                    </td>
                    <td style={{ padding: "8px 10px", textAlign: "center" }}>
                      {row.shorepower ? <span style={{ color: "#059669", fontSize: 12 }}>✓</span> : <span style={{ color: "#CBD5E1" }}>—</span>}
                    </td>
                    <td style={{ padding: "8px 10px", color: "var(--gf-muted)", fontSize: 11, whiteSpace: "nowrap" }}>{row.eta}</td>
